...
# Build the first row of dataframe
temp <- data.frame(N = N, M = M, n = n, nMN = n*M/n, SampMean = 0)
ui <- tagList(
...
server <- function(session, input, output) {
    observe({
        ...
        # Update E for first row of dataframe
        if(nrow(Final) == 1){
            Final$SampMean = E
        }

        # Check duplicates
        new <- data.frame(N = N, M = M, n = n, nMN = n*M/n, SampMean = E)
        
        if(nrow(merge(Final, new))==0){  # Now Intersect
